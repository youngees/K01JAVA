...
class ShowThread extends Thread 
{
	...
	Thread 클래스의 run() 메서드를 오버라이딩 한것으로 해당 메서드는
	...
	public void run()
	{
		for(int i=1 ; i<=100 ; i++) {
			...
			try {
				/*
				Thread클래스의 static메서드로 실행의 흐름을 주어진
				시간만큼 block상태로 전환시켜 준다.
				1000분의 1초 단위로 기술하게 되므로 아래는 0.01초만큼
				block시키게 된다. 즉 1000이 1초이다.
				 */
				sleep(10);
			}
...
public class Ex01ThreadStart
{
	public static void main(String[] args)
	{
		...
		/*
		run()메소드를 통해 호출은 단순한 실행만 될뿐 쓰레드가
		생성되지 않는다. 아래의 경우 첫번째 쓰레드객체의 모든작업이
		완료된 후 두번째 쓰레드가 동작한다.
		 */
//		st1.run();
//		st2.run();
